...
class VisionTransformer(nn.Module):
    ...
    def _init_pos_embed(self, pos_embed):
        ...
        grid_size = (self.img_height // self.patch_size, self.img_width // self.patch_size)
        if self.is_video:
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_activation_checkpointing:
                x = torch.utils.checkpoint.checkpoint(
                    blk, x, masks, attn_mask, T=T, H_patches=H_patches, W_patches=W_patches, use_reentrant=False
                )
            else:
    ...
    def interpolate_pos_encoding(self, x, pos_embed):
        ...
        if self.is_video:
            ...
            init_h = self.img_height // self.patch_size
            init_w = self.img_width // self.patch_size
            target_h = H // self.patch_size
            target_w = W // self.patch_size

            pos_embed = nn.functional.interpolate(
                pos_embed.reshape(1, init_h, init_w, dim).permute(0, 3, 1, 2),
