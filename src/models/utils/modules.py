...
def build_action_block_causal_attention_mask(T, H, W, add_tokens=1):
    ...
    # 1. 创建时间维度的因果掩码 (T, T) -> [[1, 0], [1, 1]] 这种形式
    # 使用 torch.tril 直接在 CPU 上生成下三角矩阵，极快
    time_mask = torch.tril(torch.ones(T, T, dtype=torch.bool))

    # 2. 创建空间/Token维度的全 1 掩码 (N_T, N_T)
    block_mask = torch.ones(N_T, N_T, dtype=torch.bool)

    # 3. 使用 Kronecker 积快速扩展矩阵
    # 这步由 PyTorch 底层 C++ 实现，比 Python 循环快几个数量级
    mask = torch.kron(time_mask, block_mask)
