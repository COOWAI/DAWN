...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            x_cls, output = model(x, t_input, **model_kwargs)
        else:
            x_cls, output = model(x, t_input, cond, **model_kwargs)
        if model_type == "noise":
            return x_cls, output
        elif model_type == "x_start":
            alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
            return x_cls, (x - expand_dims(alpha_t, x.dim()) * output) / expand_dims(sigma_t, x.dim())
        elif model_type == "v":
            alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
            return x_cls, expand_dims(alpha_t, x.dim()) * output + expand_dims(sigma_t, x.dim()) * x
        elif model_type == "score":
            sigma_t = noise_schedule.marginal_std(t_continuous)
            return x_cls, -expand_dims(sigma_t, x.dim()) * output
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            sigma_t = noise_schedule.marginal_std(t_continuous)
            x_cls, noise = noise_pred_fn(x, t_continuous)
            print(cond_grad[:, 0, :])
            print(noise[:, 0, :])
            noise_ = noise - guidance_scale * expand_dims(sigma_t, x.dim()) * cond_grad
            print(noise_[:, 0, :])
            print(sigma_t)
            return x_cls, noise - guidance_scale * expand_dims(sigma_t, x.dim()) * cond_grad
            ...
            if guidance_scale == 1.0 or unconditional_condition is None:
                ...
                x_cls, noise_uncond = noise_pred_fn(x, t_continuous, cond=unconditional_condition)
                x_cls, noise = noise_pred_fn(x, t_continuous, cond=condition)
                return x_cls, noise_uncond + guidance_scale * (noise - noise_uncond)
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        """
        x_cls, noise = self.noise_prediction_fn(x, t)
    ...
    def sample(
        ...
        with torch.no_grad():
            if method == "adaptive":
                ...
                t = timesteps[step]
                t_prev_list = [t]
                x_cls, model_prev = self.model_fn(x, t)
                ...
                for step in range(1, order):
                    ...
                    model_prev_list.append(self.model_fn(x, t)[1])
                ...
                for step in range(order, steps + 1):
                    ...
                    if step < steps:
                        model_prev_list[-1] = self.model_fn(x, t)[1]
            ...
            if denoise_to_zero:
                ...
                x_cls, x = self.denoise_to_zero_fn(x, t)
                if self.correcting_xt_fn is not None:
